...
const Form = ({ currentId, setCurrentId }) => {
  const [postData, setPostData] = useState({ title: '',price: '', message: '', tags: [], selectedFile: '' });
  ...
  const clear = () => {
    setCurrentId(0);
    setPostData({ title: '',price: '', message: '', tags: [], selectedFile: '' });
  };
    ...
    return (
      <Paper className={classes.paper} elevation={6}>
        <Typography variant="h6" align="center">
          Please Signin to Sell your Product.
        </Typography>
      </Paper>
    );
  }
  ...
  return (
    <Paper className={classes.paper} elevation={6}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editing "${post?.title}"` : 'Sell Product'}</Typography>
        ...
        <Button className={classes.buttonSubmit} variant="contained" color="primary" size="large" type="submit" fullWidth>Sell</Button>
